...
abstract class rowToX(val session: Session,val alogger: Logger) {
  ...
  */
  ...
  val prepTickersWidths = session.prepare("""select ticker_id,bar_width_sec from mts_meta.bars_property where ticker_id=1 and bar_width_sec=300;""")
  ...
  case class TickersWidth(ticker_id :Int,bar_width_sec :Int) {
    ...
    val maxTsEndBars = seqBars.map(sb => sb.ts_end).max
  ...
  case class TickerProperty(ticker_id :Int, bar_width_sec :Int,deepCntPattern :Int){
    ...
    val patternForSearch = new PatternForSearchCls(seqBars.slice(seqBars.size-deepCntPattern, seqBars.size-1) :+ seqBars.last)

    def getPatternForSearch = {patternForSearch}

    val searchArea : Seq[BarC] = seqBars.takeWhile(b => b.ts_end < patternForSearch.seqBarsSrc.head.ts_end)

    def getSearchArea = {searchArea}

    /*
    def funcCompareCurrSeqAndCurrPattern(currBarFromFSeq : Seq[BarC]) = {
      val currCmpareBarsGroups :PatternForSearchCls = new PatternForSearchCls(currBarFromFSeq)

      val res = for ((bp,idx) <- currCmpareBarsGroups.getSeqBarCpat.zipWithIndex) yield {
        alogger.info("    index=  "+idx+" "+ bp.b.ts_end +
          " prcntTicks " + bp.ticks_cnt_prcnt +
          " prcntLogCO " + bp.abs_logco_prcnt
        )
        if (patternForSearch == currCmpareBarsGroups)
          currCmpareBarsGroups
      }
      res
    }
    */
    ...
    def getSearchResult :Seq[Seq[BarC]] = {
      val seqOfSeq = for (i <- 0.to(searchArea.size-patternForSearch.getSeqBarCpat.size)) yield {
        //alogger.info("DEVIDE PARTS from "+i+" to "+(i+patternForSearch.getSeqBarCpat.size-1))
        searchArea.slice(i, (i+patternForSearch.getSeqBarCpat.size-1)) :+ searchArea(i+patternForSearch.getSeqBarCpat.size-1)
      }
      val res = seqOfSeq.filter(sb => patternForSearch == new PatternForSearchCls(sb))
      res
    }

    /*
    def getAvgLogCO = {
      val posSumLogCO = searchArea.filter(b => b.log_co>0).map(b => b.log_co).sum
      val negSumLogCO = searchArea.filter(b => b.log_co<0).map(b => b.log_co).sum

      val posCnt      =  searchArea.filter(b => b.log_co>0).map(b => b.log_co).size
      val negCnt      =  searchArea.filter(b => b.log_co<0).map(b => b.log_co).size

      (posSumLogCO/posCnt, negSumLogCO/negCnt)
    }
    */
